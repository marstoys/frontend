              ...
              <div className='w-[900px] flex items-center gap-[24px]'>
                <ul className='w-[126px] flex flex-col gap-[20px]'>
                  {product.images?.map((image, index) => (
                    <li key={index} onClick={() => handleThumbnailClick(index)} className={`w-[80px] h-[80px] rounded-[12px] overflow-hidden cursor-pointer border-2 transition-all ${selectedImageIndex === index ? 'border-blue-500 scale-110' : 'border-gray-200'}`} >
                  ...
                  <li key={index} className='w-full'>
                    <div className='flex justify-between mb-[14px]'>
                      <div className='flex flex-col'>
                        <h3 className='font-medium text-[18px] text-[#3E3E3E]'>{item.first_name}</h3> {/* first_name */}
                        <p className='font-light text-[14px] text-[#3E3E3E]'>{item.created_at}</p> {/* created_at */}
                      </div>
                      <ul className='flex items-center gap-[3px]'>
                        {[...Array(5)].map((_, i) => (
                          <li key={i} className="hover-rotate">
                            <StarFilled className={i < (item.rating || 0) ? '!text-[#FFC117]' : '!text-[#E0E0E0]'} />
                          </li>
                        ))}
                      </ul>
                    </div>
                    <p className='font-regular text-[16px] text-[#3E3E3E]'>{item.comment}</p> {/* comment */}
                  </li>
                ))}
              </ul>
                  ...
                  >
                    <div className='flex items-center gap-[20px]'>
                      ...
                      <div>
                        ...
                        <p className='text-[16px] font-medium text-[#3E3E3E]'>
                          Jami: <span className='text-pink-500'>{(typeof item.price === 'string' ? parseInt(item.price) : item.price) * item.quantity} so'm</span>
                        </p>
                      </div>
                    </div>
                    <div className='flex items-center gap-[20px] '>
        ...
        <div className='w-full mx-auto py-[50px] block md:hidden px-6'>
          ...
          {basketCount > 0 ? (
            <>
              <div className='w-full flex flex-col gap-[20px] mb-[30px]'>
                {basketItems?.map((item) => (
                  <div key={item.id} className='w-full flex flex-col py-[20px] px-5 border rounded-[10px] bg-white shadow-lg'>
                    ...
                    <div className='flex items-center gap-[30px] mx-10 mt-5'>
                      <div className='flex items-center gap-2'>
